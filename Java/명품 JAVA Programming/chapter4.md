...
static 멤버를 가진 클래스의 객체가 생성되는 과정을 통해 static 멤버와 non-static 멤버에 대해 보다 자세히 알아보자. 아래 그림의 StaticSample 클래스는 2개의 static 멤버를 가지고 있다.

![](./img/chapter4/ex30.jpg)

- static 멤버의 생성

  위 그림에서 static 멤버가 생성되는 시점은 StaticSample이 사용되기 시작하는 시점(StaticSample 클래스가 로딩되는 시점)이다.  
  다음 코드가 실행되는 시점에는 static 멤버 m과 f()는 이미 존재하며 사용이 가능하다.
  ...
  static 멤버 m과 f()는 이들 두 객체가 생성되기 이전에 이미 생성되어 있으므로, s1과 s2 객체가 생성될 때 인스턴스 멤버인 n, g(), h()만 객체마다 생성된다.
...
- static 멤버의 공유
  ...
  위 그림에서 객체 s1과 객체 s2는 static 멤버 m과 f()를 공유하고 또한 자신의 멤버라고 생각한다. g(), h()에서도 static 멤버 m을 공유하고 있는 것을 볼 수 있다.
